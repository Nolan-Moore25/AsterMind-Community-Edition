...
**Verified during planning (2026-07-21):** these files are not merely un-imported — they are already **non-compiling** in place. `src/synth/examples/quickstart.ts` imports `'../synth/index.js'` (resolves to `src/synth/synth/`, which does not exist — the real barrel is `src/synth/index.ts`, one level up), and `src/synth/examples/evaluateGeneratedData.ts` imports `'../scripts/loadTrainingData'` when `src/synth/scripts/` contains only `pipeline.ts`. So NB-001's "orphaned" characterization is if anything understated: this is dead code that does not resolve its own imports today. Crucially, **no file under `src/` imports from either directory** (verified by grep), so their state — broken or not — has zero effect on `npm run build` or `npm test`. This changes how §2(d) treats them (move-and-flag, byte-identical, not "relocate working scripts") but not *whether* to move them.
...
**Why `node_examples/` moves inside `examples/` rather than just getting renamed in place.** The actual problem isn't the name alone — it's that "every runnable example lives under `examples/`" isn't true today, and a top-level sibling with a suspiciously similar name is the most confusing possible way for that to be false. Nesting it resolves both the naming collision and the structural inconsistency in one move. Its own `package.json`, `package-lock.json`, and `tsconfig.json` move with it and it stays a self-contained sub-package — with **one required edit**: `node_examples/package.json` declares `"@astermind/astermind-elm": "file:.."`, a local-path dependency on the repo-root package. Moving the sub-package one directory deeper changes what `..` resolves to (from the repo root to `examples/`), so that entry must become `"file:../.."` for the local dependency to keep pointing at the root package. This is the only content edit the relocation forces on the sub-package's own files; everything else moves verbatim.

**On the moved synth scripts' broken imports (§1.4).** Because `src/synth/examples/`'s files are already non-compiling in place (§1.4) and are excluded from every build/test gate, they are moved **byte-identical** — this ADR does not fix their pre-existing broken imports, and the move will additionally invalidate `trainELMFromSynth.ts`'s `'../../core/*'` relative imports (which currently resolve) since they now point outside the relocated tree. This is deliberate: repointing imports on dead code whose sibling files already don't resolve is polish with no consumer, and the honest outcome is to relocate the reference material intact and flag it for a future fix-or-delete decision, not to half-repair it. The ADR's §6 validation is adjusted accordingly (it checks byte-identical relocation, not compilation).
...
- **NB-001 closure check:** `src/synth/examples/` and `src/synth/scripts/` no longer exist at their old paths; the moved copies under `examples/node-scripts/synth/` are byte-identical to their pre-move content (`git mv` with no edits). Note: these files do **not** compile even before the move (§1.4) and are not made to compile by it — they are relocated intact as flagged reference material, and `npm run build`/`npm test` are unaffected either way since nothing under `src/` imports them.

## 7. Sign-off Checklist

- [ ] ADR reviewed
- [x] IMPL plan written and linked ([IMPL-0009](../implementation-plans/IMPL-0009-consolidate-repo-navigation-and-naming.md))
- [x] Moves executed and grep-clean check passes (every old-path token outside intentionally-preserved historical records is gone)
- [x] Tests / guardrails in place (`npm run build` OK; `npm test` 19 files / 122 tests green; all `DEMO=` paths resolve)
- [x] Docs updated (`README.md`, `ARCHITECTURE.md`, `CONTRIBUTING.md`, `docs/*`, per-folder `README.md`/`STARTER.md`, new `examples/demos/README.md`, `claude-markdown-documents/README.md` index + syllabus cross-link)
- [x] Branch correct for the work (`repo-navigation-consolidation`)
