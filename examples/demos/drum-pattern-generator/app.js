...
function buildInputWindow(tokens, pos, N) {
  const out = new Array(N * V).fill(0); let o = 0;
...
function createConfig(engine, inputSize, hiddenUnits, ridgeLambda, activation) {
  ...
  if (engine === 'online') {
    return {
      inputDim: inputSize, outputDim: V, hiddenUnits, activation, ridgeLambda, weightInit: 'he',
      forgettingFactor: 0.997, log: { verbose: false, modelName: 'DrumOnlineELM' }
    };
  }
  return {
    inputSize, hiddenUnits, activation, ridgeLambda, task: 'classification',
...
class DrumSynth {
  ...
  kick(t) {
    const c = this.ctx, o = c.createOscillator(), g = c.createGain(); o.type = 'sine'; o.frequency.setValueAtTime(120, t); o.frequency.exponentialRampToValueAtTime(40, t + 0.15);
  ...
  snare(t) {
    const c = this.ctx, b = c.createBuffer(1, c.sampleRate * 0.2, c.sampleRate); const d = b.getChannelData(0); for (let i = 0; i < d.length; i++) d[i] = Math.random() * 2 - 1;
  ...
  hat(t) {
    const c = this.ctx, b = c.createBuffer(1, c.sampleRate * 0.05, c.sampleRate); const d = b.getChannelData(0); for (let i = 0; i < d.length; i++) d[i] = Math.random() * 2 - 1;
...
function playSequence(seq) {
  const drums = new DrumSynth(); const bpm = 120, spb = 60 / bpm, stepDur = spb / 4; const ctx = drums.ctx; const start = ctx.currentTime + 0.2;
